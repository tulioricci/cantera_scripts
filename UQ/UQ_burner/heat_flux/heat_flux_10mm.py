...
volume_per_min = 25
mdot_rate = 0.1693 # initial guess, will be modified during runtime
...
for mechanism in ["uiuc_7sp", "uiuc_20sp", "wang99_51sp"]: #, "wang99_75sp", "gri30", "Blanquart2018"]: #, "AramcoMech3.0"]:
    ...
    for phi in phi_array:
        ...
        if os.path.exists(filename):
            ...
            print('m =', u_int*A_int/lmin_to_m3s,', phi =', phi, ', Tb =', T[0], T[-1], u_int)
            ...
            u_int = volume_per_min*lmin_to_m3s/A_int
            rhoU_int = rho_int*u_int  
            mdot_int = rho_int*u_int*A_int
            ...
            header = "grid, u, T, D, " + species_output + "_" + (', ' + species_output + '_').join(str(x) for x in gas.species_names) + ", mu, kappa"
...
if "wang99_75sp" in my_dict.keys():
...
ax1.plot(  cantera_uiuc_7sp[:,0],  -cantera_uiuc_7sp[:,4], '-.', lw=3.0, color='dodgerblue', marker='X', markersize=8, label='7sp')
ax1.plot( cantera_uiuc_20sp[:,0], -cantera_uiuc_20sp[:,4], '-.', lw=3.0, color='forestgreen', marker='P', markersize=8, label='20sp')
ax1.plot( cantera_wang_51sp[:,0], -cantera_wang_51sp[:,4], '-.', lw=3.0, color='tomato', marker='d', markersize=6, label='51sp')
if "wang99_75sp" in my_dict.keys():
    ax1.plot( cantera_wang_75sp[:,0], -cantera_wang_75sp[:,4], '-.', lw=3.0, color='red', marker='s', markersize=6, label='75sp')
if "gri30" in my_dict.keys():
    ax1.plot( cantera_gri30[:,0], -cantera_gri30[:,4], '-.', lw=3.0, color='purple', marker='s', markersize=6, label='GRI 3.0')
if "Blanquart2018" in my_dict.keys():
    ax1.plot( cantera_blanq[:,0], -cantera_blanq[:,4], '-.', lw=3.0, color='gold', marker='s', markersize=6, label='Blanquart')
if "AramcoMech3.0" in my_dict.keys():
    ax1.plot( cantera_aramco[:,0], -cantera_aramco[:,4], '-.', lw=3.0, color='silver', marker='s', markersize=6, label='Aramco')
...
for text in legend.get_texts():
    ...
    if kk == 8:
        break
fig_name = '1D_burner_heat_flux' + '_m' + str('%4.2f' % volume_per_min) + "_" + transp_model
if enable_soret:
...
ax1.set_xlim(0.48, 1.32)
ax2.set_xlim(0.48, 1.32)
ax1.set_ylim(4.0, 7.0)
ax3.set_ylim(4.0, 7.0)
ax1.set_xlabel(r'$\bf{Equivalence \; ratio} \; \Phi$')
ax1.set_ylabel(r'$\bf{Heat \;  flux \; (W/cm^2)}$')
ax1.legend(loc='upper left', bbox_to_anchor=(-0.01,1.01), fontsize=13)
##ax3.legend(loc='upper left', Bbox_to_anchor=(-0.01,0.79), fontsize=13)
...
for text in legend.get_texts():
    ...
    if kk == 8:
        break
fig_name = '2D_burner_heat_flux' + '_m' + str('%4.2f' % volume_per_min) + "_" + transp_model
if enable_soret:
    fig_name = fig_name + "_soret"
if use_radiation:
    fig_name = fig_name + "_radiation"
...
ax1.plot( cantera_uiuc_7sp[:,0],  cantera_uiuc_7sp[:,2], '-.', lw=3.0, color='dodgerblue', marker='X', markersize=8, label='7sp')
ax1.plot(cantera_uiuc_20sp[:,0], cantera_uiuc_20sp[:,2], '-.', lw=3.0, color='forestgreen', marker='P', markersize=8, label='20sp')
ax1.plot(cantera_wang_51sp[:,0], cantera_wang_51sp[:,2], '-.', lw=3.0, color='tomato', marker='d', markersize=6, label='51sp')
if "wang99_75sp" in my_dict.keys():
    ax1.plot(cantera_wang_75sp[:,0], cantera_wang_75sp[:,2], '-.', lw=3.0, color='red', marker='s', markersize=6, label='75sp')
if "gri30" in my_dict.keys():
    ax1.plot( cantera_gri30[:,0], cantera_gri30[:,2], '-.', lw=3.0, color='purple', marker='s', markersize=6, label='GRI 3.0')
if "Blanquart2018" in my_dict.keys():
    ax1.plot( cantera_blanq[:,0], cantera_blanq[:,2], '-.', lw=3.0, color='gold', marker='s', markersize=6, label='Blanquart')
if "AramcoMech3.0" in my_dict.keys():
    ax1.plot( cantera_aramco[:,0], cantera_aramco[:,2], '-.', lw=3.0, color='silver', marker='s', markersize=6, label='Aramco')
